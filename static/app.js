...
/* AI STUDY MENTOR — conversational dashboard guidance */
function mentorContext(question){
  return `You are DHANU STUDY OS, a calm and practical AI study mentor talking directly with a student.
Student question: ${question}
Subjects and topics: ${subjectTopicText()||'none added yet'}
Weak topics from practice: ${state.weakTopics.join(', ')||'none recorded'}
Latest confidence: ${state.confidence}/10
Focus time: ${state.focus} minutes
Tasks completed: ${state.done}
Give a direct, student-friendly answer. Do not overwhelm the student. If they are stuck, explain the idea in simple steps, then give one small action or question to check understanding. Do not pretend to know missing syllabus details; ask for the topic when needed.`;
}
function addMentorBubble(text, role){
  const chat=$('mentor-chat'); if(!chat)return;
  const row=document.createElement('div'); row.className=`mentor-row mentor-row-${role}`;
  row.innerHTML=role==='ai'
    ? `<div class="mentor-avatar">M</div><div class="mentor-bubble">${escapeHtml(text)}</div>`
    : `<div class="mentor-bubble">${escapeHtml(text)}</div>`;
  chat.appendChild(row); chat.scrollTop=chat.scrollHeight;
}
async function askMentor(question){
  const input=$('mentor-input'), form=$('mentor-form'), status=$('mentor-status'), error=$('mentor-error');
  if(!question||!form)return;
  form.classList.add('is-thinking'); if(input)input.disabled=true; error?.classList.add('hidden');
  addMentorBubble(question,'student');
  if(status)status.textContent='Your mentor is thinking about the best next step…';
  try{
    const r=await fetch('/api/coach',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({context:mentorContext(question)})});
    const data=await r.json(); if(!r.ok)throw new Error(data.message||'The study mentor is temporarily unavailable.');
    addMentorBubble(data.message||'Tell me a little more about what you are studying.','ai');
    if(status)status.textContent=data.mode==='AI mode'?'AI mentor · personalised to your study data':'Study mentor · ready with a practical next step';
  }catch(e){
    if(error){error.innerHTML=`Couldn’t reach the mentor. <button id="mentor-retry" class="text-btn">Retry →</button>`;error.classList.remove('hidden');$('mentor-retry')?.addEventListener('click',()=>askMentor(question));}
    if(status)status.textContent='Mentor connection needs a retry.';
  }finally{
    form.classList.remove('is-thinking'); if(input){input.disabled=false;input.focus();}
  }
}
$('mentor-form')?.addEventListener('submit',(e)=>{
  e.preventDefault();
  const input=$('mentor-input'); const q=input?.value.trim();
  if(!q)return;
  input.value=''; askMentor(q);
});
